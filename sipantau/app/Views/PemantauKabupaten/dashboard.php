...
<div class="card">
    ...
    <div id="datacontent-kepatuhan" class="data-tab-content hidden">
        <div id="kepatuhanStatsCards" class="grid grid-cols-2 lg:grid-cols-5 gap-3 lg:gap-4 mb-6"></div>
...
<script>
    ...
    async function loadPetugas(page = 1) {
        ...
        // Deteksi desktop atau mobile untuk value filter
        ...
        if (!kegiatanId) {
            ...
            `;
            document.getElementById('petugasTableBody').innerHTML = emptyHtml;
        ...
        try {
            // Menggunakan AbortController untuk membatalkan request sebelumnya jika user mengetik cepat
            if (abortController) abortController.abort();
            abortController = new AbortController();

            const response = await fetch(
                `${basePetugas}?id_kegiatan_wilayah=${kegiatanId}&page=${page}&perPage=${perPage}&search=${encodeURIComponent(search)}`,
                { signal: abortController.signal }
            );
            const result = await response.json();

            if (result.success) {
                // Support backend lama (jika backend belum support pagination, kita pakai data array langsung)
                let data = [];
                let pagination = null;
                ...
                if (result.pagination) {
                    ...
                    // Fallback jika backend belum update, anggap result.data adalah semua data
                    data = result.data;
                ...
                petugasDataCache = data;
                renderPetugasTable(data);
                renderPetugasCards(data);
                
                if (result.pagination) {
                    renderPetugasPagination(result.pagination);
                    updatePetugasDataInfo(data.length, result.pagination.total, result.pagination.current_page, result.pagination.per_page);
                }
            }
        } catch (e) {
    ...
    function renderKepatuhanStats(stats) {
        ...
        <div class="card hover:shadow-lg transition-shadow duration-200 col-span-2 lg:col-span-1">
            <div class="flex items-center justify-between">
                <div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        document.getElementById('filterKegiatan').addEventListener('change', function () {
            ...
            document.getElementById('searchPetugasInputMobile').value = '';
