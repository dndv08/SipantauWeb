...
<!-- Back Button & Title -->
<div class="mb-6">
    <a href="<?= base_url('pemantau-provinsi/laporan-progress-petugas') ?>"
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200">
    ...
    <div class="border-b border-gray-200">
        <nav class="flex -mb-px">
            <button onclick="switchTab('progress')" id="tabProgress"
    ...
    <div class="p-6">
        <!-- Pantau Progress Tab -->
        <div id="contentProgress" class="tab-content">
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-gray-50 border border-gray-200">
                            ...
                            <th class="px-4 py-3 text-center text-xs font-semibold border-r border-gray-200 text-gray-700">Realisasi Absolut</th>
                    ...
                    <tbody class="divide-y divide-gray-200" id="progressTableBody">
                        <tr>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between mt-6 gap-4" id="progressPagination"></div>
        </div>
...
<script>
    const idPCL = <?= $idPCL ?>;
    const baseUrl = '<?= base_url('pemantau-provinsi/laporan-progress-petugas') ?>';
    ...
    function switchTab(tab) {
        ...
        if (tab === 'progress' && !tabDataCache.progress) loadPantauProgress(1);
    ...
    async function loadPantauProgress(page = 1) {
        ...
        try {
            ...
            if (result.success) {
                renderProgressTable(result.data, result.pagination);
                if (page === 1) tabDataCache.progress = true;
            } else {
    ...
    function renderProgressTable(data, pagination) {
        const tbody = document.getElementById('progressTableBody');
        if (data.length === 0) {
        ...
        renderPagination('progress', pagination);
    }
    ...
    function renderPagination(type, pagination) {
        ...
        for (let i = 1; i <= pagination.totalPages; i++) {
            buttons += i === pagination.currentPage ?
                ...
                `<button onclick="load${type === 'progress' ? 'PantauProgress' : 'LaporanTransaksi'}(${i})" class="px-3 py-1 border rounded text-sm hover:bg-gray-50">${i}</button>`;
        }
